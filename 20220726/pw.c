  ...
  while (1) {
    ...
    */
    ...
    for (i = 0, j = 0; read(clientfd, hbuffer + i, 1); i++) {
      ...
      if (hbuffer[i] == ':' && !headers[j].value && j > 0) {
        hbuffer[i] = 0;
        headers[j].value = hbuffer + i + 2;
      }
    }

    // printing all the headers
    // getting the Accept-Language header if it exists
    int accept_language_header_index = -1;
    for (i = 0; i < j; i++) {
      if (strcmp(headers[i].name, "Accept-Language") == 0) {
        accept_language_header_index = i;
      }
    ...
    */
    if (!strcmp(method, "GET")) {
      ...
      if ((strcmp(requested_language, "it") == 0) || (strcmp(requested_language, "fr") == 0) || (strcmp(requested_language, "de") == 0) || (strcmp(requested_language, "es") == 0)) {
        valid_language = 1;
      }
      ...
      if (strcmp("www.openoffice.org", hostname) == 0 && valid_language) {
        ...
        // Initialize the new string with the filename
        strcpy(filename_with_language, filename);
        printf(ANSI_COLOR_MAGENTA "FILENAME_WITH_LANGUAGE: %s\n" ANSI_COLOR_RESET, filename_with_language);
        // Append the '/' and the language
        strcat(filename_with_language, "/");
        printf(ANSI_COLOR_MAGENTA "FILENAME_WITH_LANGUAGE: %s\n" ANSI_COLOR_RESET, filename_with_language);
        strcat(filename_with_language, requested_language);
        printf(ANSI_COLOR_MAGENTA "FILENAME_WITH_LANGUAGE: %s\n" ANSI_COLOR_RESET, filename_with_language);
      }

      // make HTTP request to the host
      sprintf(request, "GET /%s HTTP/1.1\r\nHost:%s\r\nConnection:close\r\n\r\n", filename_with_language, hostname);
