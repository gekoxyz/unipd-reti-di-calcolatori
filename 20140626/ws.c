...
int main() {
  ...
  /*
  Gestire l’header ​Content-Length ​descritto nella RFC 1945 alla sezione 10.4
  */
  ...
  while (1) {
    ...
    if (fork() == 0) {
      ...
      if (fin == NULL) {
        ...
        char wc_command[1024];
        sprintf(wc_command, "wc -m .%s", filename);
        fp = popen(wc_command, "r");
